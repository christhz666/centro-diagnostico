...
def listar_facturas():
    ...
    facturas = query.order_by(Factura.fecha_factura.desc()).limit(50).all()
...
def obtener_factura(factura_id):
    ...
    detalles = [{'id': d.id, 'descripcion': d.descripcion, 'cantidad': d.cantidad, 'precio_unitario': float(d.precio_unitario), 'total': float(d.total)} for d in factura.detalles]
    pagos = [{'id': p.id, 'fecha_pago': p.fecha_pago.isoformat(), 'monto': float(p.monto), 'metodo_pago': p.metodo_pago} for p in factura.pagos]
...
def registrar_pago(factura_id):
    try:
        ...
        return jsonify({'success': True, 'message': 'Pago registrado', 'pago': {'id': pago.id, 'monto': float(pago.monto), 'metodo_pago': pago.metodo_pago, 'fecha_pago': pago.fecha_pago.isoformat()}}), 201
    except ValueError as e:
        return jsonify({'success': False, 'error': str(e)}), 400
    except Exception as e:
        db.session.rollback()
...
@bp.route('/estado-cuenta/<int:paciente_id>', methods=['GET'])
@jwt_required()
def estado_cuenta_paciente(paciente_id):
    facturas = Factura.query.filter_by(paciente_id=paciente_id, estado__ne='anulada').all()
    total_facturado = sum(float(f.total) for f in facturas)
    total_pagado = sum(sum(float(p.monto) for p in f.pagos) for f in facturas)
    return jsonify({'total_facturado': total_facturado, 'total_pagado': total_pagado, 'saldo_pendiente': total_facturado - total_pagado})
